...
logistic_regression_model = LogisticRegression(C=30.0, penalty='l2', solver='liblinear', max_iter=2000, random_state=42)
svm_model = SVC(C=30.0, kernel='rbf', gamma='scale', probability=True, random_state=42, max_iter=5000)
...
prob_train = model.predict_proba(X_train_corr_scaled)[:, 1]
prob_test = model.predict_proba(X_test_corr_scaled)[:, 1]
prob_blind = model.predict_proba(X_blind_corr_scaled)[:, 1]

pred_train = model.predict(X_train_corr_scaled)
pred_test = model.predict(X_test_corr_scaled)

results = []

name = "ensemble_logreg_svc"
...
# final dataframe
results_df = pd.DataFrame(results, columns=["model", "Dataset", "Accuracy", "AUROC", "Recall", "Specificity", "F1"])
